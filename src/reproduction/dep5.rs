...
/// The DEP-5 `debian/copyright` rendering of an
/// [`Attribution`](crate::reproduction::Attribution).
///
/// The runtime counterpart of the build half's own DEP-5 renderer:  the
/// same packages [`Markdown`](crate::reproduction::Markdown) renders, in the
/// machine-readable format a Debian distribution ships instead of a prose
/// `THIRDPARTY.md`.  A test holds the two renderers together.
#[derive(Clone, Copy, Debug)]
pub struct Dep5<'a>(pub &'a crate::Attribution);
...
impl Dep5<'_> {
    ...
    fn body(
        ...
        for licence in package.licences {
            write!(f, " .\n {} ({})", licence.identifier, licence.origin)?;
            f.write_str(":\n .\n")?;
            Self::fold(f, licence.text)?;
        }
    ...
    /// Writes the mandatory `Copyright` field.
    ///
    /// The embedded model carries no author data — a package manifest is
    /// the only source, and it does not survive the harvest — so the
    /// absence is stated rather than guessed at.
    fn copyright(f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    ...
    fn files(
        ...
        let identifiers = package
            .licences
            .iter()
            .map(|licence| licence.identifier)
            .collect::<Vec<_>>()
            .join(" and ");
